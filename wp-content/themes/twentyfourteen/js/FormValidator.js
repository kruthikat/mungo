    ...
    var rangeOfCharacter = function(key, value) {
        ...
        if (!validationObj.hasOwnProperty(key)) {
            if (fieldLength > value.maxlength) {
                $("#" + key).addClass('error');
                validationObj[key] = value.msg + " should not be greater than " + value.maxlength;
            } else {
    ...
    init = function(settings) {
        ...
        $.each(settings, function(i, v) {
            if (v.hasOwnProperty('required') == true && v.required == true)
                required(i, v);
            if (v.hasOwnProperty('selectRequired') == true && v.selectRequired == true)
                selectRequired(i, v);
...
function registerValidate() {
    ...
    if(MyValidator.init(obj)) {
    	if(jQuery("#rpcnpwd").val() != jQuery("#rpnpwd").val()) {
    		jQuery("#rpcnpwd").parent().append("<p class='error'>Passwords don't match</p>");
    		return false;
    	}
    	else {
